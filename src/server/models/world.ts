...
const initialObjects: Record<string, WorldObject> = {
  soul_shard: {
    ...
    shortDescription: "A thumbnail-sized crystal that pulses with a faint violet light, warm against your palm.",
    longDescription:
  ...
  veyllas_ribbon: {
    ...
    shortDescription: "A slip of midnight-blue silk tied around your wrist, carrying the scent of night-blooming jasmine.",
    longDescription:
...
const initialLocations: Record<string, Location> = {
  the_velvet_thorn: {
    ...
    shortDescription: "A brothel nestled deep in the Warrens — smoke, silk, and whispered promises.",
    longDescription:
  ...
  matt_harbor_lower: {
    ...
    attributes: {
      ...
      "Notable Locations": "Slave markets, fighting pits, the Sinking Dock tavern, the Velvet Thorn brothel",
...
const initialCharacters: Record<string, Character> = {
  veyla: {
    ...
    type: "CHARACTER",
    displayName: "Veyla",
    shortDescription: "A half-elf courtesan with eyes like burnished gold — equal parts savior and temptation.",
    longDescription:
  ...
  madam_cressida: {
    ...
    shortDescription: "Proprietor of the Velvet Thorn — a woman who has seen everything and forgotten nothing.",
    longDescription:
...
export function seedDatabase() {
  ...
  if (count.count === 0) {
    ...
    // Seed the root plot — the player's awakening and the mystery of their power
    db.prepare(
      ...
      "IN_PROGRESS",
      JSON.stringify(["the_velvet_thorn", "matt_harbor_lower"]),
      JSON.stringify(["veyla", "madam_cressida"]),
      null,
      null,
      JSON.stringify([
        { plotId: null, triggerCondition: "Player deepens their bond with Veyla and explores the nature of their magic through their connection" },
        { plotId: null, triggerCondition: "Player navigates the Warrens underworld — Harbor Rats, rogue mages, black markets — to find answers" },
        { plotId: null, triggerCondition: "Player seeks knowledge in the upper levels, risking exposure to the Magewardens and the duke's law" },
      ]),
    );
    console.log("Seeded initial plot: The Awakening.");

    // Set initial game time to late night (atmospheric for waking in a brothel)
    ...
    // Set initial scene — player's room at the Velvet Thorn
    db.prepare("INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)").run(
      "current_scene",
      JSON.stringify({
