...
export class ContextAssembler {
  ...
  async assemble(
    ...
    const {
      query, maxItems = 10,
      includeShortTerm = true, includeLongTerm = true, includeReasoning = true,
    } = options || {};

    const context: AssembledContext = {
      messages: [], entities: [], preferences: [], traces: [], summary: "",
    };
    ...
    if (includeShortTerm && sessionId) {
      tasks.push(
        this.shortTerm.getConversation(sessionId, maxItems)
    ...
    if (includeLongTerm && query) {
      tasks.push(
        this.longTerm.searchEntities(query, { limit: maxItems })
          .then(entities => {
            context.entities = entities.map(({ similarity: _, ...e }) => e);
          }),
      );
      tasks.push(
        this.longTerm.getPreferences(undefined, maxItems)
    ...
    if (includeReasoning && query) {
      tasks.push(
        this.reasoning.getSimilarTraces(query, { limit: 3 })
          .then(traces => { context.traces = traces.map(({ similarity: _, ...t }) => t); }),
      );
    }
