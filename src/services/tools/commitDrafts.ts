...
export const createCommitDraftsTool = (callbacks: { 
  ...
  execute: async ({ reasoning }: { reasoning: string }) => {
    try {
      ...
      return `Failed to commit drafts due to database or backend error: ${error.message}. You must review the drafts and tell the GM to fix the error. DO NOT try to commit again until the GM provides new drafts.`;
    }
  }
});
