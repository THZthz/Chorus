const READ_ALLOWED_LABELS = new Set([
  "Entity", "Message", "NPCDisposition", "GameTime",
]);
...
const ALLOWED_RELATIONSHIPS = new Set([
  "LOCATED_AT", "CARRIES", "ALLIED_WITH", "HOSTILE_TOWARDS",
  "LOCATED_IN", "HAS_DISPOSITION", "HAS_MESSAGE", "FIRST_MESSAGE", "NEXT_MESSAGE",
]);

const BLOCKED_READ_CLAUSES = /\b(CREATE|MERGE|DELETE|SET|REMOVE|DETACH\s+DELETE|DROP)\b/i;
const BLOCKED_DDL = /\b(CREATE\s+INDEX|CREATE\s+CONSTRAINT|DROP\s+(INDEX|CONSTRAINT|DATABASE|GRAPH)|ALTER|USING\s+PERIODIC\s+COMMIT)\b/i;
...
export class CypherValidator {
  validateRead(query: string): ValidationResult {
    ...
    if (BLOCKED_READ_CLAUSES.test(query)) {
      errors.push("Query contains write clause (CREATE/MERGE/DELETE/SET/REMOVE/DETACH DELETE/DROP). queryWorld is read-only.");
    }
    ...
    if (UNBOUNDED_PATH.test(query)) {
      errors.push("Query contains unbounded variable-length path (*). Use a fixed upper bound like [*1..3].");
    }
  ...
  validateWrite(query: string): ValidationResult {
    ...
    if ((hasDelete || hasDetachDelete) && !this.hasQualifiedMatch(query)) {
      errors.push("DELETE/DETACH DELETE must be preceded by a specific MATCH with at least one property condition (WHERE clause or property specification).");
    }
  ...
  private hasQualifiedMatch(query: string): boolean {
    return /\bMATCH\b.*\bWHERE\b/i.test(query)
      || /\bMATCH\b[^}]*\{[^}]*:/i.test(query);
  }
}
